...
class YOLO(object):
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        for i, c in list(enumerate(top_label)):
            ...
            label_size = (label_bbox[2] - label_bbox[0], label_bbox[3] - label_bbox[1])  # 计算宽度和高度
    ...
    def gumbel_softmax_symbolic(g, logits, tau=0.5, hard=False):
        # 使用 ONNX 的 Softmax 操作符
        softmax = g.op("Softmax", logits, axis_i=1)
        return softmax

    # 注册自定义符号函数
    torch.onnx.register_custom_op_symbolic('custom::gumbel_softmax', gumbel_softmax_symbolic, 11)
